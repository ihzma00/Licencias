...
namespace Licencias.Module.E2E.Tests;

public class LicenciasTests : IDisposable {
    const string BlazorAppName = "LicenciasBlazor";
    const string WinAppName = "LicenciasWin";
    const string AppDBName = "Licencias";
    EasyTestFixtureContext FixtureContext { get; } = new EasyTestFixtureContext();

	public LicenciasTests() {
        FixtureContext.RegisterApplications(
            new BlazorApplicationOptions(BlazorAppName, string.Format(@"{0}\..\..\..\..\Licencias.Blazor.Server", Environment.CurrentDirectory)),
            new WinApplicationOptions(WinAppName, string.Format(@"{0}\..\..\..\..\Licencias.Win\bin\EasyTest\net8.0-windows\Licencias.Win.exe", Environment.CurrentDirectory))
        );
        FixtureContext.RegisterDatabases(new DatabaseOptions(AppDBName, "LicenciasEasyTest", server: @"(localdb)\mssqllocaldb"));	           
	}
    public void Dispose() {
